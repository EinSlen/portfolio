import React from "react"
import Link from "next/link"
import { Button } from "@/components/ui"
import { MouseScrollAnimation } from "./mouse-scroll-animation"
import {basePath} from "@/utils";
const data = {
    title: {
        1: [{ text: "Hey !" }],
        2: [
            { text: "Je suis " },
            { text: "Valentin damlencourt", highlighted: true },
            { text: "." },
        ],
        3: [{ text: "Voici mon portfolio." }],
    },
    subtitle: "Retrouvez mes projets et mes compétences à travers ce portfolio.",
    callToActions: [
        {
            label: "Télécharger mon CV",
            href: `${basePath}/assets/cv.pdf`,
        },
        {
            label: "Explore my projects",
            href: "#projects",
        },
    ],
};

// Définit un type pour les clés de title
type TitleKeys = 1 | 2 | 3;

const HeroBanner = () => {
    return (
        <div className="relative flex min-h-[calc(100vh-64px)] items-center justify-center p-4">
            <div className="pb-28">
                <h1 className="font-display mb-2 text-center text-3xl md:text-4xl lg:text-5xl xl:text-6xl">
                    {Object.keys(data.title).map((sectionKey) => {
                        const key = Number(sectionKey) as TitleKeys; // Convertit sectionKey en TitleKeys
                        return (
                            <span key={sectionKey}>
                                {data.title[key].map(
                                    (item: { text: string; highlighted?: boolean }, index: number) => (
                                        <span
                                            key={index}
                                            className={`${item?.highlighted ? "text-primary" : ""} text-9xlxl font-bold`}
                                        >
                                            {item.text}
                                        </span>
                                    )
                                )}
                                <br />
                            </span>
                        );
                    })}
                </h1>
                <h2 className="mb-8 text-center text-lg md:text-xl">{data.subtitle}</h2>
                <div className="flex justify-center space-x-2 sm:space-x-4">
                    {data.callToActions.map((elem, index) => (
                        <Button
                            key={index}
                            asChild
                            ariaLabel="Call action"
                            size="lg"
                            variant={index === 0 ? "default" : "neutral"}
                        >
                            <Link href={elem.href} scroll={true}>
                                {elem.label}
                            </Link>
                        </Button>
                    ))}
                </div>
            </div>
            <div className="absolute bottom-5 left-1/2 translate-x-[-50%]">
                <MouseScrollAnimation />
            </div>
        </div>
    );
};

export default HeroBanner;
